...
/*
中断控制函数
void NVIC_AD(uint32_t group,uint8_t main,uint8_t sub,uint8_t IRQchannel)
group  向量组 NVIC_PriorityGroup_1
main   主优先级  1
sub    次优先级  1
IRQchannel 中断通道 ADC1_2_IRQn ADC1和ADC2的全局中断
*/
void NVIC_AD(uint32_t group,uint8_t main,uint8_t sub,uint8_t IRQchannel)
{
	NVIC_InitTypeDef NVIC_InitStructure;	// 优先级分组
	NVIC_PriorityGroupConfig(group);
  NVIC_InitStructure.NVIC_IRQChannel = IRQchannel;  // 配置中断优先级
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = main;
...
void ADC_INIT_Single_Channel(uint8_t ADCx,uint8_t Channel)
{
	...
	switch(ADCx)
	{
		case 1:
			...
			ADC_SoftwareStartConvCmd(ADC1, ENABLE); //软件触发AD		
			break;
		case 2:
			...
			RCC_ADCCLKConfig(RCC_PCLK2_Div8); //分频9MHz
				ADC_RegularChannelConfig(ADC2, Channel_Set, 1, 
	                         ADC_SampleTime_55Cycles5);//配置采样时间
			...
			ADC_SoftwareStartConvCmd(ADC2, ENABLE); //软件触发AD			
			break;
		case 3:
			...
			ADC_SoftwareStartConvCmd(ADC3, ENABLE); //软件触发AD				
			break;
	}
	NVIC_AD(NVIC_PriorityGroup_1,1,1,ADC1_2_IRQn);
...
void ADC_INIT_Multiple_Channels(uint8_t ADCx,uint8_t CH_NUM,uint8_t GPIOx)
{
	...
	switch(ADCx)
	{
		case 1:
			...
			DMA_InitStructure.DMA_BufferSize = CH_NUM;// 缓冲区大小，应该等于数据目的地的大小  转换通道个数
			...
			DMA_Cmd(ADC1_DMA_Channel , ENABLE);	// 使能 DMA 通道
		...
		case 3:
			...
			DMA_InitStructure.DMA_PeripheralBaseAddr = ( u32 ) ( & ( ADC1->DR ) );	// 外设基址为：ADC 数据寄存器地址
			DMA_InitStructure.DMA_MemoryBaseAddr = (u32)ADC3_ConvertedValue;	// 存储器地址
			DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralSRC;	// 数据源来自外设
			DMA_InitStructure.DMA_BufferSize = CH_NUM;// 缓冲区大小，应该等于数据目的地的大小  转换通道个数
			DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;	// 外设寄存器只有一个，地址不用递增
			DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable; 	// 存储器地址递增
			...
			DMA_InitStructure.DMA_Priority = DMA_Priority_High;	// DMA 传输通道优先级为高，当使用一个DMA通道时，优先级设置不影响
			DMA_InitStructure.DMA_M2M = DMA_M2M_Disable;	// 禁止存储器到存储器模式，因为是从外设到存储器
			DMA_Init(ADC3_DMA_Channel, &DMA_InitStructure);	// 初始化DMA
			DMA_Cmd(ADC3_DMA_Channel , ENABLE);	// 使能 DMA 通道
			break;
	}
	/*****************************GPIO_ADC_MODE_SET*********************************/
...
在中断函数里调用
*/
uint16_t ADC_Get_Single_Channel()
{
	if (ADC_GetITStatus(ADC2,ADC_IT_EOC)==SET) 
	{
		// 读取ADC的转换值	
		ADC_Single_IT_Get = ADC_GetConversionValue(ADC2);
	}
	ADC_ClearITPendingBit(ADC2,ADC_IT_EOC);

	return ADC_Single_IT_Get;
}
